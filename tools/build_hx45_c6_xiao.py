#!/usr/bin/env python3
"""Build the isolated HX4.5 image for the Seeed Studio XIAO ESP32C6."""
...
TEMPLATE = ROOT / "tests/hardware-in-loop/hx45-c6-seeed-xiao-4m"
DEFAULT_SEALED_NATIVE_ELF = (
    TEMPLATE / "fixtures/synthetic-loopback-esp32c6.elf"
)
MATRIX = ROOT / "firmware/idf-family-matrix.json"
LOCK = ROOT / "firmware/locks/host-extension/idf-5.4.4/esp32c6/dependencies.lock"
...
PROJECT_NAME = "pulse_hx45_c6_xiao_4m"
REPORT_SCHEMA = "pulse.esp32.hx45-c6-xiao-build.v1"
APPLICATION_PARTITION_BYTES = 2 * 1024 * 1024
# The guest declares a 128 KiB linear memory. Keep an additional 32 KiB of
# static-link headroom so a build which cannot even make that contiguous
# allocation never reaches the physical gate.
...
def validate_sealed_sources() -> dict[str, Any]:
    ...
    if sha256(LOCK) != EXPECTED_DEPENDENCY_LOCK_SHA256:
        raise HardwareBuildError("C6 dependency lock no longer matches the HX4.5 seal")
    return {
        "firmware": firmware,
...
def validate_sdkconfig(path: Path) -> None:
    ...
    forbidden = (
        ...
        "CONFIG_ELF_LOADER_CACHE_OFFSET",
    ...
    if mismatches:
        raise HardwareBuildError("invalid generated XIAO C6 sdkconfig: " + "; ".join(mismatches))
...
def prepare_native_elf(
    ...
    if sealed_native_elf is None:
        ...
        elf = native_dir / "synthetic-loopback-esp32c6.elf"
...
def qualify_build(
    *,
    ...
    if not TEMPLATE.is_dir() or not LOCK.is_file() or not COMMON_WASM.is_file():
        raise HardwareBuildError("XIAO C6 template, component lock, or common Wasm is missing")
    ...
    report: dict[str, Any] = {
        ...
        "native_extension": {
            "elf": file_evidence(native_elf, "native-extension/synthetic-loopback-esp32c6.elf"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    native_source.add_argument(
        ...
        help="Use only the exact HX4.5-sealed C6 ELF instead of rebuilding it",
    )
    native_source.add_argument(
        "--rebuild-native-elf",
        action="store_true",
    ...
    try:
        ...
        print(f"XIAO C6 HX4.5 build failed: {exc}", file=sys.stderr)
        return 1
    print(
        json.dumps(
            {
