#!/usr/bin/env python3
"""Evaluate retained serial evidence from the HX4.5 XIAO ESP32C6 harness."""
...
EXPECTED_BOARD = "Seeed Studio XIAO ESP32C6"
EXPECTED_BOARD_MARKING = "XIAO ESP32C6"
...
EXPECTED_DEPENDENCY_LOCK_SHA256 = (
    "edfeaa76d16c1eb9e8f60e46e0a342af41bf238b1b1226a7c0c0b541f35e228a"
)
...
MINIMUM_LINK_DIRAM_REMAIN_BYTES = 160 * 1024
EXPECTED_FLASH_BYTES = 4 * 1024 * 1024
EXPECTED_PSRAM_BYTES = 0
...
REPORT_SCHEMA = "pulse.esp32.hx45-c6-xiao-hardware-report.v1"
...
def evaluate(
    *,
    ...
    if build.get("schema") != "pulse.esp32.hx45-c6-xiao-build.v1":
    ...
    if (
        ...
        or claim.get("canonical_compile_realization_unchanged") is not True
    ):
    ...
    if not isinstance(build_board, dict):
        build_board = {}
    if (
        build_board.get("manufacturer") != "Seeed Studio"
        or build_board.get("model") != "XIAO ESP32C6"
        or build_board.get("board_marking") != EXPECTED_BOARD_MARKING
        or build_board.get("target") != "esp32c6"
        or build_board.get("flash_bytes") != EXPECTED_FLASH_BYTES
        or build_board.get("psram_bytes") != EXPECTED_PSRAM_BYTES
    ):
        failures.append("build report does not describe the exact Seeed Studio XIAO lane")
    ...
    normalized_marking = re.sub(r"[^A-Z0-9]", "", board_marking.upper())
    normalized_expected = re.sub(r"[^A-Z0-9]", "", EXPECTED_BOARD_MARKING)
    if normalized_marking != normalized_expected:
        failures.append(
            f"board marking is {board_marking!r}, expected {EXPECTED_BOARD_MARKING!r}"
        )
    ...
    if native.get("sha256") != EXPECTED_NATIVE_ELF_SHA256:
        failures.append("native extension does not match the HX4.5 C6 seal")
    ...
    if (
        ...
        failures.append("dependency lock does not match the HX4.5 C6 seal")
    ...
    for board in boards:
        if board.get("board") != EXPECTED_BOARD or board.get("target") != "esp32c6":
            failures.append("serial board identity does not match the XIAO C6 lane")
        if board.get("flash_bytes") != EXPECTED_FLASH_BYTES:
            failures.append("observed flash size is not 4 MB")
        if board.get("psram_bytes") != EXPECTED_PSRAM_BYTES:
            failures.append("observed PSRAM size is not zero")
    ...
    for cycle in measured:
        ...
        if cycle.get("psram_before") != 0 or cycle.get("psram_after") != 0:
            failures.append(f"measured cycle {cycle.get('cycle')} reported PSRAM")
    ...
    required_cases = {
        "board-is-esp32c6": (2, 1),
        "board-flash-is-4mb": (2, 1),
        "board-has-no-psram": (2, 1),
    ...
    if (
        ...
        or any(value != 0 for value in psram_samples)
        or final.get("minimum_psram_free") != 0
    ):
        failures.append("no-PSRAM heap evidence is not consistently zero")
    if (
    ...
    if (
        ...
        or any(value != 0 for value in psram_largest_samples)
        or final.get("minimum_psram_largest") != 0
    ):
        failures.append("no-PSRAM largest-block evidence is not consistently zero")
    ...
    report: dict[str, Any] = {
        ...
        "board": {
            ...
            "model": "XIAO ESP32C6",
            "board_marking": board_marking,
            "identity_basis": "OPERATOR_ATTESTED_BOARD_MARKING_PLUS_MEASURED_CHIP_AND_FLASH",
            "target": "esp32c6",
        ...
        "claim_boundary": {
            "proves": [
                "ESP32-C6 target ELF inspection, relocation, and descriptor execution",
                ...
                "internal free/largest-block floors plus extension/main-task stack headroom",
                "target WAMR trap handling",
                "quiescence timeout, reset-required latch, software reset, and retained harness breadcrumb",
            ],
            "does_not_prove": [
                "electronic authentication of the Seeed Studio PCB vendor",
                "the canonical compile-only C6 realization or production partition layout",
                "GPIO, networking, BLE, IEEE 802.15.4, OTA, or power-loss behavior",
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    title = (
        "# HX5b Seeed Studio XIAO ESP32C6 pressure qualification"
        if report["pressure_seal"]["required"]
        else "# HX4.5 Seeed Studio XIAO ESP32C6 hardware qualification"
    )
    lines = [
        title,
        "",
        ...
        f"- Board: `{board['model']}`",
        f"- Board marking: `{board['board_marking']}`",
        ...
        "## Claim boundary",
        "",
        "This is named-board runtime evidence for the isolated 4 MB/no-PSRAM HIL lane. "
        "It does not qualify the production partition realization or any physical peripheral.",
        "",
    ]
    if report["failures"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--board-marking", required=True)
    ...
    try:
        ...
        report = evaluate(
            ...
            board_marking=args.board_marking,
        ...
        print(f"XIAO C6 HX4.5 evaluation failed: {exc}", file=sys.stderr)
        return 1
