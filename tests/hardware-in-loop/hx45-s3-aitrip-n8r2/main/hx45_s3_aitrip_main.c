...
#error "The AITRIP HX4.5 harness is valid only for ESP32-S3"
#endif

#define HX45_BOARD_NAME "AITRIP ESP32-S3-DevKitC-1 N8R2"
#define HX45_FLASH_BYTES (8u * 1024u * 1024u)
#define HX45_PSRAM_BYTES (2u * 1024u * 1024u)
...
#define HX45_PSRAM_FREE_MIN_BYTES (512u * 1024u)
#define HX45_PSRAM_LARGEST_MIN_BYTES (256u * 1024u)
...
static uint32_t s_minimum_psram_free = UINT32_MAX;
static uint32_t s_minimum_psram_largest = UINT32_MAX;
...
static HeapSample heap_sample(void)
{
    ...
    if (sample.psram_free < s_minimum_psram_free) {
        s_minimum_psram_free = sample.psram_free;
    }
    if (sample.psram_largest < s_minimum_psram_largest) {
        s_minimum_psram_largest = sample.psram_largest;
    }
    return sample;
}
...
static bool record_heap_headroom(const HeapSample *sample)
{
    ...
    passed = record_condition("psram-free-headroom",
                              sample->psram_free >=
                                  HX45_PSRAM_FREE_MIN_BYTES) && passed;
    passed = record_condition("psram-largest-headroom",
                              sample->psram_largest >=
                                  HX45_PSRAM_LARGEST_MIN_BYTES) && passed;
    return passed;
}
...
static bool load_and_start_extension(uint32_t cycle)
{
    ...
    wdc_extension_profile_esp32s3(&profile);
...
static bool run_clean_cycle(uint32_t cycle, bool measured, CycleResult *out)
{
    ...
    if (measured) {
        ...
        passed = record_condition("psram-heap-recovered",
                                  out->psram_recovered != 0u) && passed;
    }
...
static bool run_real_wasm_trap(void)
{
    ...
    passed = record_condition("trap-psram-heap-recovered",
                              before.psram_free == after.psram_free) && passed;
...
static bool board_preflight(uint32_t *out_flash, uint32_t *out_psram)
{
    ...
    uint32_t psram_size = (uint32_t)esp_psram_get_size();
    esp_chip_info(&chip);
    ...
    *out_psram = psram_size;
    printf("PULSE_HX45_BOARD {\"board\":\"%s\",\"target\":\"esp32s3\""
    ...
    fflush(stdout);
    return record_condition("board-is-esp32s3", chip.model == CHIP_ESP32S3) &&
           record_condition("board-flash-is-8mb", flash_size == HX45_FLASH_BYTES) &&
           record_condition("board-psram-is-2mb", psram_size == HX45_PSRAM_BYTES);
}
