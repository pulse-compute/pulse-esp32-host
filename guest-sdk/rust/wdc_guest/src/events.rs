...
pub use crate::abi::{
    ...
    WDC_EVENT_HTTP_RESPONSE, WDC_EVENT_MODULE_PROBATION_ENDING, WDC_EVENT_MODULE_PROBATION_STARTED,
    WDC_EVENT_MQTT_MESSAGE, WDC_EVENT_NETWORK_STATUS_CHANGED, WDC_EVENT_NET_CONNECTED,
    WDC_EVENT_NET_DISCONNECTED, WDC_EVENT_SHUTDOWN_REQUEST, WDC_EVENT_TIMER_FIRED,
};
...
pub fn decode_event_envelope(buf: &[u8]) -> Result<EventEnvelope<'_>, i32> {
    ...
    let causation_id = match encoding::find_u64(buf, WDC_CBOR_KEY_CAUSATION_ID) {
        Ok(id) => Some(id),
        Err(WDC_ERR_NOT_AVAILABLE) => None,
        Err(e) => return Err(e),
    };
